    ...
    <div className="min-h-screen bg-gray-50">
      <main className="max-w-7xl mx-auto px-4 py-10">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-4">
          ...
          <Link to="/admin/user/create" className="bg-primary text-black px-8 py-3 rounded-2xl font-bold shadow-lg hover:opacity-90 transition active:scale-95 flex items-center gap-2 border-2 border-white">
            <FiPlus /> New Customer
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {customers?.map((customer) => (
            <div key={customer._id} className="bg-white rounded-3xl p-8 shadow-md border border-gray-100 hover:shadow-xl transition-all relative overflow-hidden group">
              {/* Point Badge */}
              <div className="absolute top-0 right-0 bg-primary text-black px-6 py-2 rounded-bl-3xl font-black text-lg shadow-sm">
                {customer.points} <span className="text-[10px] uppercase font-bold ml-1 opacity-60">pts</span>
              </div>

              <div className="flex items-center gap-4 mb-6">
                <div className="w-14 h-14 bg-gray-50 rounded-2xl flex items-center justify-center text-gray-400 text-2xl group-hover:bg-primary group-hover:text-black transition-colors border border-gray-100">
                  <FiUser />
                </div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-8">
                <button
                  onClick={() => {
                    const points = prompt("Enter points to give:", "10");
                    if (points && !isNaN(points)) {
                      earnPoints.mutate({ customerId: customer._id, pointsToAdd: parseInt(points) });
                    }
                  }}
                ...
                >
                  ...
                  <span className="text-[10px] uppercase tracking-tighter">Give Points</span>
                </button>
